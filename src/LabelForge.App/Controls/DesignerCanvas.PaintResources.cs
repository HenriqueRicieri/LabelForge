...
{
    private readonly Dictionary<(double? Millimeters, bool Dark), FormattedText> _rulerLabels = [];
    ...
    {
        var key = (millimeters is { } mm ? Math.Round(mm) : (double?)null, IsDark);
        ...
        label = new FormattedText(key.Item1?.ToString(CultureInfo.InvariantCulture) ?? "mm",
